...
def group_feed(group_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    posts = list(
        db.scalars(
            select(SharePost)
            .where(SharePost.group_id == group_id)
            .options(selectinload(SharePost.items))
            .order_by(SharePost.created_at.desc())
            .limit(50)
        ).all()
    )
    ...
    combined_items = [
        ...
        for (name, unit), qty in sorted(combined.items(), key=lambda kv: (-kv[1], kv[0][0]))
        if qty > 0
    ]
        ...
        "posts": [
            {
                ...
                "items": [{"name": i.name, "quantity": i.quantity, "unit": i.unit} for i in p.items],
            }
            for p in posts
        ],
