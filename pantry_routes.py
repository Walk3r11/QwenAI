...
def add_item(payload: PantryItemCreateRequest, db: Session=Depends(get_db), current_user: User=Depends(get_current_user)):
    item = PantryItem(user_id=current_user.id, name=payload.name.strip(), quantity=payload.quantity, unit=payload.unit.strip() if payload.unit else None, expires_at=payload.expires_at, source='manual')
    db.add(item)
    db.commit()
    db.refresh(item)
    return item
