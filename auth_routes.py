...
def signup(payload: SignupRequest, db: Session=Depends(get_db)):
    ...
    if existing:
        ...
        db.commit()
        if not send_verification_email(existing.email, existing.name, code):
            raise HTTPException(status_code=503, detail='Failed to send verification email. Check SMTP settings.')
    ...
    if instant:
        tok = create_access_token(user.id)
        return AuthResponse(access_token=tok, token_type='bearer', user=UserOut.model_validate(user))
    if not send_verification_email(user.email, user.name, code):
        raise HTTPException(status_code=503, detail='Failed to send verification email. Check SMTP settings.')
