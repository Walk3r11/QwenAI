...
def health():
    ...
    return {'ok': True, 'db_connected': db_ok, 'using_neon': 'neon.tech' in DATABASE_URL, 'jwt_configured': bool(JWT_SECRET), 'ai_enabled': ENABLE_AI, 'model_file_present': model_present if ENABLE_AI else None, 'mmproj_file_present': mmproj_present if ENABLE_AI else None, 'groq_configured': groq_configured(), 'freshness_min': FRESHNESS_MIN, 'freshness_max': FRESHNESS_MAX, 'freshness_default': FRESHNESS_DEFAULT, 'allow_pc_script_signup': ALLOW_PC_SCRIPT_SIGNUP, 'email_configured': bool(os.getenv('SMTP_PASSWORD')) and bool(os.getenv('SMTP_USER') or os.getenv('SMTP_EMAIL'))}
